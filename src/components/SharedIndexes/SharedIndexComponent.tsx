                    ...
                    <Card variant="outlined">
                        <CardContent>
                            ...
                            <Typography variant="body2" color="text.secondary" mb={2}>
                                ...
                                {params.row.publication?.map((pub: {publisher: string, dateOfPublication: string}) => pub.publisher+" "+pub.dateOfPublication).join(', ')}
                            </Typography>
                            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap mb={2}>
                                { params.row.subjects?.map((sub: {value:string},index:bigint) => (
                                    <Chip key={params.row.id+"."+index+"."+sub.value} label={sub.value} />
                                ))}
                            </Stack>
                            <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap mb={2}>
                                { params.row.isbns?.map((isbn:string,index:bigint) => (
                                    <Chip key={params.row.id+"."+index+"."+isbn} label={`isbn ${isbn}`} />
                                ))}
                                { params.row.issns?.map((issn:string,index:bigint) => (
                                    <Chip key={params.row.id+"."+index+"."+issn} label={`issn ${issn}`} />
                                ))}
                            </Stack>
        ...
        <Box sx={{ width: "100%" }}>
            ...
            <DataGridPremium
                ...
                sx={{
                    ...
                    '& .MuiDataGrid-cell': { borderBottom: 'none', borderTop: 'none', padding: 0 },
                    border: 'none',
                    '& .MuiDataGrid-row': {
