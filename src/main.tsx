...
async function startStandalone(): Promise<void> {
	...
	const { getStandaloneConfig, mountDcbAdminForLibraries } = await import(
		"./application"
	);

	const rootElement = document.getElementById("root");
	if (!rootElement) {
		throw new Error("DCB Admin for Libraries root element is missing");
	}

	const config = await getStandaloneConfig();
	await mountDcbAdminForLibraries(rootElement, config);
}
