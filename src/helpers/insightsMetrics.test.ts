...
describe("the metric registry", () => {
	it("finds the panels it is meant to be checking", () => {
		...
		// an empty list.
		expect(panels.length).toBeGreaterThanOrEqual(18);
	...
	it("counts the panels that still explain nothing", () => {
		...
		// A number, not an intention. Eight panels render a figure with no statement of
		// where it came from; their queries have not been read against the SQL, and
