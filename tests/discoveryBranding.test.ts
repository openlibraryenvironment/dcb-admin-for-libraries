 ...
 * half of that contract: the prop on BrandImageField existed and defaulted to true, and
 * nothing ever passed it, so a store=none deployment offered a button that could only 404.
 */
describe("brandAssetStoreFrom", () => {
	...
	it("returns null when the branding block is absent at any level", () => {
		...
		expect(brandAssetStoreFrom({ dcb: { branding: { assets: {} } } })).toBe(null);
	});
