...
DCB Admin for Libraries / the next release (assessed against `main`; set the
number when it is cut)

**Report Date**

This report was last updated on 22/09/26.

**Product Description**

DCB Admin for Libraries is a modern web application for library staff working with the OpenRS DCB resource sharing software.

It has two modes: one for library administrators, and one for library workers. The latter mode is limited to the placing of staff requests only.
...
DCB Admin for Libraries is open-source software. It utilises the Material UI open-source component library and benefits from many of its accessibility features, while also extending these to go beyond the standard.
...
Automated testing is a continuous gate rather than a one-off exercise: `e2e/accessibility.spec.ts` runs axe-core against the WCAG 2.0, 2.1 and 2.2 A and AA rule sets on every pipeline, over ten surfaces in both light and dark colour schemes, and the build fails on any violation. The pages covered are listed in that file. It asserts axe’s *incomplete* results too, not only its violations.

Two things automated rules cannot see are checked by tests of our own, because both were real defects here rather than hypotheses:

* `tests/themeContrast.test.ts` computes the contrast ratio of every colour token against the ground it actually lands on, per scheme, and fails below the floor — 4.5:1 for light and dark, 7:1 for high contrast. The axe gate only sees what a scanned page renders; this sees what is declared, including tokens used only in an error state.
* `tests/attributePairs.test.ts` prevents a field label being rendered as text beside its value with nothing connecting the two. There is no broken relationship for axe to detect in that pattern, only an absent one.

**Where we know we fall short.** Wide data grids still require horizontal scrolling. Charts carry their numbers as a table but their *shape* is not described in words. No external audit has been commissioned; everything in this report is our own assessment, and the tests named in it are the part a reader can check for themselves.
...
| [**1.3.1 Info and Relationships**](http://www.w3.org/TR/WCAG20/#content-structure-separation-programmatic) (Level A) | Supports | Landmarks, heading order and list semantics are used throughout, and are asserted by the axe gate. Field labels on the detail pages are description lists (`dl`/`dt`/`dd`), so a label and its value are announced as a pair; before September 2026 they were adjacent text with nothing connecting them, which no automated rule can detect. `tests/attributePairs.test.ts` prevents the pattern returning. |
...
| [**1.3.3 Sensory Characteristics**](http://www.w3.org/TR/WCAG20/#content-structure-separation-understanding) (Level A) | Supports | DCB Admin for Libraries pages do not depend solely on sensory characteristics such as shape, size, visual location, orientation, or sound to understand content. Icons may be displayed to control functionality, but adjacent visible text labels or WAI-ARIA labels are also provided to ensure that relationships are available to assistive technologies.  |
| [**1.4.1 Use of Color**](http://www.w3.org/TR/WCAG20/#visual-audio-contrast-without-color) (Level A) | Supports | Colour is not the only means of conveying information: form errors carry text and `aria-invalid` alongside the colour change, and request outcomes carry a text label as well as a colour. |
...
| [**2.1.1 Keyboard**](http://www.w3.org/TR/WCAG20/#keyboard-operation-keyboard-operable) (Level A) | Supports | At the time of writing, all components in this application are accessible by keyboard alone. We continue to test releases of DCB Admin for Libraries to identify any regressions. |
...
| [**2.2.1 Timing Adjustable**](http://www.w3.org/TR/WCAG20/#time-limits-required-behaviors) (Level A) | Supports | This occurs when a user places a ‘staff request’, and an alert pops up with a link to the newly placed request. While this timing isn’t adjustable, the user can easily navigate to the placed request if they miss the initial alert. By default the most recently placed request is at the top of the page. |
...
| [**2.3.1 Three Flashes or Below Threshold**](http://www.w3.org/TR/WCAG20/#seizure-does-not-violate) (Level A) | Supports | DCB Admin for Libraries does not contain any content that flashes. |
| [**2.4.1 Bypass Blocks**](http://www.w3.org/TR/WCAG20/#navigation-mechanisms-skip) (Level A) | Supports | A skip link is the first thing the keyboard reaches on every page and moves focus to `main`. The header and tab navigation are a repeated block, so the earlier answer — that there were none — was wrong rather than merely incomplete. `e2e/landmarks.spec.ts`. |
| [**2.4.2 Page Titled**](http://www.w3.org/TR/WCAG20/#navigation-mechanisms-title) (Level A) | Supports | All pages in the DCB Admin for Libraries app have unique titles that describe the purpose of the page. |
| [**2.4.3 Focus Order**](http://www.w3.org/TR/WCAG20/#navigation-mechanisms-focus-order) (Level A) | Supports | The meaning and/or operation of the content in DCB Admin for Libraries is unaffected by focus order. |
...
| [**2.5.1 Pointer Gestures**](https://www.w3.org/TR/WCAG21/#pointer-gestures) (Level A 2.1 and 2.2) | Not applicable | DCB Admin for Libraries does not contain any functionality that uses multipoint or path-based gestures for operation. |
| [**2.5.2 Pointer Cancellation**](https://www.w3.org/TR/WCAG21/#pointer-cancellation) (Level A 2.1 and 2.2) |  Supports |  All functionality that can be single pointer operated in DCB Admin for Libraries can also be cancelled using standard browser and operating system methods. |
| [**2.5.3 Label in Name**](https://www.w3.org/TR/WCAG21/#label-in-name) (Level A 2.1 and 2.2) |  Supports |  Where controls contain visible text, that text is also the label for the control in DCB Admin for Libraries. When we become aware of deficiencies, we work to repair instances that do not meet this criterion. We conduct regular testing to catch regressions. |
| [**2.5.4 Motion Actuation**](https://www.w3.org/TR/WCAG21/#motion-actuation) (Level A 2.1 and 2.2) |  Supports | DCB Admin for Libraries does not contain any functionality that can be operated by device motion or user motion. |
| [**3.1.1 Language of Page**](http://www.w3.org/TR/WCAG20/#meaning-doc-lang-id) (Level A) | Supports | The product is English and says so on the root element. It ships ONE language deliberately: a 0.4%-complete Spanish bundle was removed in September 2026 because i18next renders a missing key as the key itself. What a second language needs before it is offered — including writing `documentElement.lang` on change — is recorded in `docs/theming.md`. |
| [**3.2.1 On Focus**](http://www.w3.org/TR/WCAG20/#consistent-behavior-receive-focus) (Level A) | Supports | DCB Admin for Libraries components that receive focus do not initiate a change of context. |
| [**3.2.2 On Input**](http://www.w3.org/TR/WCAG20/#consistent-behavior-unpredictable-change) (Level A) | Supports | Generally, changing the setting of any DCB Admin for Libraries UI component doesn’t automatically trigger a change of context. |
...
| [**3.3.7 Redundant Entry**](https://www.w3.org/TR/WCAG22/#redundant-entry) (Level A 2.2 only) | Supports | DCB Admin for Libraries does not require users to enter information more than once in the process without the information being available for the user to select. |
| [**4.1.2 Name, Role, Value**](http://www.w3.org/TR/WCAG20/#ensure-compat-rsv) (Level A) | Supports | DCB Admin for Libraries pages generally use standard HTML and WAI-ARIA attributes to describe the identity, operation, and state of user interface elements to assistive technologies. When we become aware of deficiencies in meeting this criterion, we work to repair them. |
...
| [**1.4.3 Contrast (Minimum)**](http://www.w3.org/TR/WCAG20/#visual-audio-contrast-contrast) (Level AA) | Supports | 4.5:1 or better on every text token, in every colour scheme. Measured rather than asserted: `tests/themeContrast.test.ts` computes the ratio for each ink against the ground it actually lands on and fails the build on any pair below the floor, and the axe gate scans what is rendered. This row previously read “4:5:1 throughout” and was not true — several tokens sat between 1.1:1 and 3.9:1, including field labels on the home page in dark mode, which is what a member library reported in September 2026. |
...
| [**1.4.11 Non-text Contrast**](https://www.w3.org/TR/WCAG21/#non-text-contrast) (Level AA 2.1 and 2.2) |  Supports | 3:1 for controls, focus rings and boundaries, in every scheme. Verified by the same arithmetic test and by the axe gate on every build, rather than by a one-off developer-tools check. |
...
| [**2.4.5 Multiple Ways**](http://www.w3.org/TR/WCAG20/#navigation-mechanisms-mult-loc) (Level AA) | Supports | Multiple ways of navigation are available to the user. Tab-based navigation is the primary method, built on the MUI Tab component. |
...
| [**2.5.7 Dragging Movements**](https://www.w3.org/TR/WCAG22/#dragging-movements) (Level AA 2.2 only) | Supports | There is no functionality in DCB Admin for Libraries that requires dragging movements. All interactive elements are operable using single pointer actions. |
| [**2.5.8 Target Size (Minimum)**](https://www.w3.org/TR/WCAG22/#target-size-minimum) (Level AA 2.2 only) | Supports | Targets meet 24×24 CSS px, including the icon-only actions in dense grid rows, which are the usual offender. Checked by hand — no automated rule covers it. |
| [**3.1.2 Language of Parts**](http://www.w3.org/TR/WCAG20/#meaning-other-lang-id) (Level AA) | Supports |  |
| [**3.2.3 Consistent Navigation**](http://www.w3.org/TR/WCAG20/#consistent-behavior-consistent-locations) (Level AA) | Supports | Navigation is consistent throughout, with tab-based navigation the primary approach. |
| [**3.2.4 Consistent Identification**](http://www.w3.org/TR/WCAG20/#consistent-behavior-consistent-functionality) (Level AA) | Supports |  |
| [**3.3.3 Error Suggestion**](http://www.w3.org/TR/WCAG20/#minimize-error-suggestions) (Level AA) | Supports | Yup and react-hook-form signal the error in text, linked to the field, describing the acceptable range. The schemas are unit-tested (`tests/schemas.test.ts`), which is how we found that an unset latitude produced Yup’s own untranslated “latitude cannot be null” and silently blocked the whole form. |
| [**3.3.4 Error Prevention (Legal, Financial, Data)**](http://www.w3.org/TR/WCAG20/#minimize-error-reversible) (Level AA) | Supports | No parts of this application cause legal or financial transactions for the user to occur. Where user-controllable data in data storage systems is modifiable (such as adding new mappings for a library’s configuration), the user is presented with a screen for reviewing their changes before making them. If server-side validation fails, the user is notified. It is also possible to configure the application in such a way that no data is modifiable. |
...
| [**4.1.3 Status Messages**](https://www.w3.org/TR/WCAG21/#status-messages) (Level AA 2.1 and 2.2) | Supports | Alerts and snackbars carry their role, and the application also has a single polite live region: route changes, search result counts and the reset of display settings are announced through it without moving focus. `e2e/announcements.spec.ts`. |

### **Table 3: Success Criteria, Level AAA**

Notes: Where present, the relevant RGAA AAA criteria is mentioned in the “Equivalent” column. DCB Admin for Libraries currently complies with **16** of the RGAA AAA criteria, and we aim to increase this number as we work to improve accessibility throughout the application.
...
| [**1.3.6 Identify Purpose**](https://www.w3.org/TR/WCAG21/#identify-purpose) (Level AAA 2.1 and 2.2) | Partially Supports |  | Every page has one `main`, a named navigation landmark and a skip link, all asserted by the axe gate, and field labels are programmatically associated with their values. Full support would need regions and controls to carry purpose beyond landmarks, which they do not yet. |
| [**1.4.6 Contrast (Enhanced**](http://www.w3.org/TR/WCAG20/#visual-audio-contrast7)) (Level AAA) | Partially supports | RGAA (AAA) 1.4.6 \- Partially supports | A high-contrast colour scheme is offered alongside light and dark, and it is held to 7:1 rather than 4.5:1: `tests/themeContrast.test.ts` applies the AAA floor to that scheme and fails the build below it. Light and dark remain AA. A user who needs AAA can select it; it is not the default, which is why this is partial rather than full support. |
...
| [**1.4.8 Visual Presentation**](http://www.w3.org/TR/WCAG20/#visual-audio-contrast-visual-presentation) (Level AAA) | Partially supports | RGAA (AAA) 10.8 \- Partially supports 10.9 \- Does not support 10.10 \- Supports  10.11 \- Does not support 10.12 \- Supports | The reader chooses a colour scheme (light, dark or high contrast), one of four text sizes, one of two spacing densities, and one of five typefaces — including Atkinson Hyperlegible Next, designed for low vision, and Lexend. All are self-hosted, so choosing one tells no third party anything. Text is generally not fully justified but we cannot certify that for all text. Text is readable without horizontal scrolling. Some lines still exceed 80 characters. Line and paragraph spacing meets the criteria throughout. |
...
| [**2.2.3 No Timing**](http://www.w3.org/TR/WCAG20/#time-limits-no-exceptions) (Level AAA) | Supports | RGAA (AAA) 13.4 \- supports | DCB Admin for Libraries does not contain any situations where a time limit is imposed upon the user. |
| [**2.2.4 Interruptions**](http://www.w3.org/TR/WCAG20/#time-limits-postponed) (Level AAA) | Supports | RGAA (AAA) 7.5 \- supports |  |
| [**2.2.5 Re-authenticating**](http://www.w3.org/TR/WCAG20/#time-limits-server-timeout) (Level AAA) | Supports | RGAA (AAA) 13.5 \- supports | When an authenticated session expires, DCB Admin for Libraries returns the user to where they were without losing any data upon re-authenticating. |
...
| [**2.3.2 Three Flashes**](http://www.w3.org/TR/WCAG20/#seizure-three-times) (Level AAA) | Supports | RGAA (AAA) 13.16 \- supports | DCB Admin for Libraries does not contain any content that flashes. |
| [**2.3.3 Animation from Interactions**](https://www.w3.org/TR/WCAG21/#animation-from-interactions) (Level AAA 2.1 and 2.2) | Supports |  | Settings offers an animation choice, and `prefers-reduced-motion` is honoured for anyone who has not made one. The preference overrides the operating system in both directions, so somebody on a shared workstation is not stuck with the machine’s setting. |
| [**2.4.8 Location**](http://www.w3.org/TR/WCAG20/#navigation-mechanisms-location) (Level AAA) | Supports | RGAA (AAA) 12.8 \- supports. 12.9 \- supports 12.12 \- supports  | Navigation menus and tabs are provided throughout the application. They are always relevant, and navigation menus always represent the page’s position in the site structure. The current page is always specified in the navigation menu. |
